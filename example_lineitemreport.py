...
def lineitem_report():
    """Creates a .xlsx report of all active lineitems
    (name, budget, status, placement ID, date of export)"""
    ...
    if client.token != 'Auth_error':
        if type(campaigns) == int and campaigns in requests.status_codes._codes.keys():
            ...
            data_frame = pd.DataFrame(columns=[
                ...
                'Budget amount',
                'Paused',
                'Line item Placement ID',
                'Date'
                ])
            for item in report:
                data_frame = data_frame.append(
                    pd.DataFrame(item, index=[0]), sort=True
                    )
            data_frame['Date'] = DATE_NOW_DMY
            writer = pd.ExcelWriter(
                f'Adform_lineitems_report__{DATE_NOW_DMY}.xlsx'
                )
